...
function host()
    ...
    local function _listen_tcp( h, host, port, telnet )
        ...
        local onExecuteNextListener = vlc.net.listen_tcp( host, port )
        local type = telnet and client_type.telnet or client_type.net;
        table.insert( listeners.tcp.list, { data = onExecuteNextListener,
                                            type = type,
                                          } )
    end
    ...
    local function _accept_and_select( h )
        ...
        if not (vlc.win and listeners.stdio) then
            ...
            if listeners.tcp then
                for _, onExecuteNextListener in pairs(listeners.tcp.list) do
                    for _, fd in pairs({onExecuteNextListener.data:fds()}) do
                    ...
                    for _, onExecuteNextListener in pairs(listeners.tcp.list) do
                        for _, fd in pairs({onExecuteNextListener.data:fds()}) do
                            if is_flag_set(pollfds[fd], vlc.net.POLLIN) then
                                local afd = onExecuteNextListener.data:accept()
                                new_client( h, afd, afd, onExecuteNextListener.type )
                                break
                            end
                        end
                    end
